...
pub(super) unsafe fn gf_mul_slice_neon(scalar: u16, data: &mut [u16]) {
    // Handle zero scalar
    if scalar == 0 {
...
#[cfg(target_arch = "aarch64")]
#[target_feature(enable = "neon")]
pub(super) unsafe fn gf_mul_slice_pmull_neon(scalar: u16, data: &mut [u16]) {
    // Handle zero scalar
    if scalar == 0 {
        for val in data.iter_mut() {
            *val = 0;
...
#[cfg(target_arch = "aarch64")]
#[target_feature(enable = "neon")]
pub(super) unsafe fn gf_muladd_pmull_neon(dst: &mut [u16], src: &[u16], scalar: u16) {
    if scalar == 0 {
        return;
    }
...
#[cfg(target_arch = "aarch64")]
#[target_feature(enable = "neon")]
pub(super) unsafe fn gf_muladd_column_neon(
...
#[cfg(target_arch = "aarch64")]
#[target_feature(enable = "neon")]
pub(super) unsafe fn bytes_to_u16_neon(bytes: &[u8], output: &mut [u16]) {
    let len = output.len();
