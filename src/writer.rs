...
//!
//! This module handles writing PAR2 packets to files according to the PAR2 specification.
//! Each packet has a 64-byte header followed by a type-specific body.
//!
//! # Packet Types
//!
//! - **Creator**: Identifies the software that created the archive
//! - **Main**: Archive metadata (block size, file list)
//! - **FileDesc**: Per-file metadata (hashes, size, name)
//! - **IFSC**: Input File Slice Checksums for block-level verification
//! - **RecoverySlice**: Reed-Solomon recovery data
//!
//! # Related Modules
//!
//! - [`crate::parser`]: Reads PAR2 packets
//! - [`crate::creator`]: High-level PAR2 file creation
...
mod tests {
    ...
    fn test_pad_to_alignment() {
        ...
        assert_eq!(pad_to_alignment(&[]), Vec::<u8>::new());
    }
