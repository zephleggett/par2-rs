...
fn test_galois_field_division() {
    ...
    assert_eq!(gf_div(1234, 1234), Some(1));

    // a / 1 = a
    assert_eq!(gf_div(5, 1), Some(5));
    assert_eq!(gf_div(1234, 1), Some(1234));

    // 0 / a = 0
    assert_eq!(gf_div(0, 5), Some(0));
    assert_eq!(gf_div(0, 1234), Some(0));

    // Division is inverse of multiplication
    let a = 123u16;
    let b = 456u16;
    assert_eq!(gf_div(gf_mul(a, b), b), Some(a));
    assert_eq!(gf_div(gf_mul(a, b), a), Some(b));
}
